...
def add_task_full(project_id, title, description, assignee_id, priority, status, due_date):
    # Assignee functionality is disabled in Version 6.5 — backend will ignore any provided assignee and set a sentinel
    if not title or title.strip() == "":
    ...
    # Force assignee to sentinel value to disable assignee behavior
    forced_assignee = 'unassigned'

    conn = data_manager.get_connection()
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO tasks (project_id, title, description, assignee_id, priority, status, due_date, created, completed)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    """, (
        project_id, 
        title.strip(), 
...
def update_project_description_web(project_id, description):
    conn = data_manager.get_connection()
    cursor = conn.cursor()
    cursor.execute(
