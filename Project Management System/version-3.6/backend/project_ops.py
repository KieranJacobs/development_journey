...
def export_project_to_csv():
    ui_helpers.header("EXPORT: PROJECT DATA TO CSV")
    ...
    export_dir = data_manager.BASE_DIR / "exports"
    export_dir.mkdir(parents=True, exist_ok=True)
    
    safe_title = "".join(c for c in selected_project['title'] if c.isalnum() or c in " _-").rstrip()
    filename = f"{safe_title}_report.csv"
    filepath = export_dir / filename
    
    with open(filepath, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        
        writer.writerow(["Project Title", "Project Status", "Task Title", "Priority", "Due Date", "Created Date", "Task Status"])
        
        if not tasks:
            writer.writerow([selected_project['title'], selected_project['status'], "No tasks", "-", "-", "-", "-"])
        else:
            for task in tasks:
                status_text = "Completed" if task['completed'] == 1 else "Incomplete"
                writer.writerow([
    ...
    print(f"\nSUCCESS: Project data exported to '{filepath}'")